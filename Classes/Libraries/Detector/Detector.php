...
class Detector
{
    ...
    */
    public static function build()
    {
        ...
        if ($pid && self::$debug) {
            ...
            return $mergedInfo;
        } elseif (@session_start() && isset($_SESSION) && isset($_SESSION[self::$sessionID]) && isset($_COOKIE) && isset($_COOKIE[self::$cookieID . '-ps'])) {
            ...
            return $mergedInfo;
        } elseif (@session_start() && isset($_SESSION) && isset($_SESSION[self::$sessionID])) {
            ...
            // send the data back to the script to be used
            return $mergedInfo;
        } elseif (($uaJSONCore = json_decode(@file_get_contents($uaFileCore))) && ($uaJSONExtended = json_decode(@file_get_contents($uaFileExtended)))) {
            ...
            // return to the script
            return $mergedInfo;
        } elseif (self::checkSpider() || (isset($_REQUEST['nojs']) && ($_REQUEST['nojs'] == 'true')) || (isset($_REQUEST['nocookies']) && ($_REQUEST['nocookies'] == 'true'))) {
            ...
            } elseif (isset($_REQUEST['nocookies']) && ($_REQUEST['nocookies'] == 'true')) {
    ...
    */
    private static function addToUAList()
    {
        ...
        $mergedInfo = (object) array_merge((array) $uaList, [self::$uaHash => self::$ua]);
                    ...
                    if (is_object($value)) {
                        foreach ($value as $vkey => $vvalue) {
                            if ($vvalue == 'probably') { // hack for modernizr
                                $value->$vkey = true;
                            } elseif ($vvalue == 'maybe') { // hack for modernizr
                                $value->$vkey = false;
                            } elseif (($vvalue == 1) || ($vvalue == 0)) {
