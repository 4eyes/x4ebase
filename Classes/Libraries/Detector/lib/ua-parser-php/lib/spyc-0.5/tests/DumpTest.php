...
{
    private $files_to_test = [];

    public function setUp()
    {
        $this->files_to_test =  ['../spyc.yaml', 'failing1.yaml', 'indent_1.yaml', 'quotes.yaml'];
    }
    ...
    {
        $dump = Spyc::YAMLDump(['item1', 'item2', 'item3']);
        $awaiting = "---\n- item1\n- item2\n- item3\n";
    ...
    {
        $dump = Spyc::YAMLDump(['a' => 1, 'b' => null, 'c' => 3]);
        $awaiting = "---\na: 1\nb: null\nc: 3\n";
        $this->assertEquals($awaiting, $dump);
    }
    ...
    {
        $dump = Spyc::YAMLDump([0 => 1, 'b' => 2, 1 => 3]);
        $awaiting = "---\n0: 1\nb: 2\n1: 3\n";
        $this->assertEquals($awaiting, $dump);
    }
    ...
    {
        $dump = Spyc::YAMLDump(['404', '405', '500']);
        $awaiting = "---\n- 404\n- 405\n- 500\n";
        $this->assertEquals($awaiting, $dump);
    }
    ...
    {
        $dump = Spyc::YAMLDump(['some' => "'Biz' pimpt bedrijventerreinen"]);
    ...
    {
        $dump = Spyc::YAMLDump(['titel'=> ['0' => '', 1 => 'Dr.', 5 => 'Prof.', 6 => 'Prof. Dr.']]);
        $awaiting = "---\ntitel:\n  0:\n  1: Dr.\n  5: Prof.\n  6: Prof. Dr.\n";
        $this->assertEquals($awaiting, $dump);
    }
    ...
    {
        $dump = Spyc::YAMLDump(['#color' => '#ffffff']);
        $awaiting = "---\n\"#color\": '#ffffff'\n";
        $this->assertEquals($awaiting, $dump);
    }
}
