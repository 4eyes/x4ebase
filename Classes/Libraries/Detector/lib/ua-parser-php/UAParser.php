    ...
    public static function parse($ua = null)
    {
        ...
        if (!$result) {
            if (($result = self::deviceParser()) && ($result->device != 'Spider')) {
                ...
                $result->uaOriginal     = self::$ua;
            } elseif (isset($result) && isset($result->device) && ($result->device == 'Spider')) {
    ...
    */
    private static function uaParser($regex)
    {
        ...
        if (preg_match('/' . str_replace('/', "\/", $regex['regex']) . '/', self::$ua, $matches)) {
            ...
            $mobileBrowsers = ['Firefox Mobile','Opera Mobile','Opera Mini','Mobile Safari','webOS','IE Mobile','Playstation Portable',
            ...
            $mobileOSs = ['Windows Phone 6.5','Windows CE','Symbian OS'];
    ...
    private static function deviceParser()
    {
        ...
        foreach ($deviceRegexes as $deviceRegex) {
            if (preg_match('/' . str_replace('/', "\/", $deviceRegex['regex']) . '/i', self::$ua, $matches)) {
                ...
                $mobileDevices  = ['iPhone','iPod','iPad','HTC','Kindle','Lumia','Amoi','Asus','Bird','Dell','DoCoMo','Huawei','i-mate','Kyocera',
                ...
                $tablets = ['Kindle','iPad','Playbook','TouchPad','Dell Streak','Galaxy Tab','Xoom'];
