...
class LogHistoryUtility
{
     ...
     * @param int $action
     *          Denotes which specific operation that wrote the entry.
     *          Use "0" when no sub-categorizing applies
     * @param int $error Flag.
     *          0 = message,
     *          1 = error (user problem),
     *          2 = System Error (which should not happen),
     *          3 = security notice (admin)
     * @param int $details_nr The message number.
     *          Specific for each $type and $action. This will make it possible to translate errormessages
     *          to other languages
     * @param string $details
     *          Default text that follows the message (in english!). Possibly translated by identification
     *          through type/action/details_nr
     * @param array $data
     *          Data that follows the log. Might be used to carry special information.
     *          If an array the first 5 entries (0-4) will be sprintf'ed with the details-text
     * @param string $tablename
     *          Table name. Special field used by tce_main.php.
     * @param mixed $recuid Record UID.
     *          Special field used by tce_main.php.
     * @param mixed $recpid
     ...
     * @param int $recUid
     */
    public static function fillMmHistoryRecords($dataHandler, $tablename, $fieldArray, $recUid)
    {
        global $TCA;
