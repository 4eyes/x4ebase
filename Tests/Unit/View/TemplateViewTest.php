...
class TemplateViewTest extends \X4e\X4ebase\Tests\Unit\Base\TestCaseBase
{
    ...
    {
        $this->mockSubject();
        $parserConfiguration = $this->getMock(\TYPO3\CMS\Fluid\Core\Parser\Configuration::class, ['addInterceptor']);
        $parserConfiguration->expects($this->once())->method('addInterceptor')->with(new \X4e\X4ebase\View\Interceptor\ReplaceTabs());

        $objectManager = $this->getMock(\TYPO3\CMS\Extbase\Object\ObjectManager::class, ['get'], [], '', false);
        ...
        $request = $this->getMock(\TYPO3\CMS\Extbase\Mvc\Request::class, ['getFormat']);
        $request->expects($this->once())->method('getFormat')->willReturn('json');

        $controllerContext = $this->getMock(\TYPO3\CMS\Extbase\Mvc\Controller\ControllerContext::class, ['getRequest']);
