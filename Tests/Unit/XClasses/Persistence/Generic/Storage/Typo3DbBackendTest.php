...
class Typo3DbBackendTest extends \X4e\X4ebase\Tests\Unit\Base\TestCaseBase
{
    ...
    public function testAddAdditionalWhereClause_QuerySettingsInstanceOfAlternativeQuerySettingsInterface_AND_RespectSysLanguageAlternative_CallsAddAlternativeSysLanguageStatement()
    {
        ...
        $querySettings = $this->getMock(
            ...
            ['getRespectSysLanguage', 'getRespectSysLanguageAlternative', 'getRespectStoragePage', 'getStoragePageIds']
        );
        $tableName = '';
        $sql = [];
    ...
    public function testDoLanguageAndWorkspaceOverlay()
    {
        ...
        $source = $this->getMock(\TYPO3\CMS\Extbase\Persistence\Generic\Qom\Selector::class, ['getSelectorName'], [], '', false);
        $source->expects($this->once())->method('getSelectorName')->willReturn($tableName);
        $databaseHandle = $this->getMock(\TYPO3\CMS\Core\Database\DatabaseConnection::class, ['exec_SELECTgetSingleRow']);
        ...
        $pageRepository = $this->getMock(\TYPO3\CMS\Frontend\Page\PageRepository::class, ['versionOL', 'getPageOverlay', 'getRecordOverlay']);
